...
var si = {
	...
	init : function(){
		...
		si.mySquare = { width: 50, height: 40, shotW: 5, shotH: 10 };
		...
		$('#level').html("Vagues: "+si.level);
		$('#monsters').html("Zombie crees: "+si.monsters);
		$('#killed').html("Tues: "+si.killed.length);
		...
		if(si.played > 0){
			...
			$('#game_object').append("<div id='info'><p>Bonjour survivant!<br /><br />Survivrez vous aux vagues de zombies	?<br /><br />Les niveaux vous conviennent? ==><br /><br /><div class='anim'><a id='restart' href='#'>Alors allons-y!</a></div></p></div>");
	...
	gameRoutine: function(){
		...
		si.myInterval = setInterval(function() {
			...
			$('#played').html("Parties: "+si.played);
			$('#lines').html("Lignes: "+si.lines);
			$('#monsters').html("Zombies crees: "+si.monsters);
			$('#shot_ratio').html("R/tirs: "+si.shotRatio.toString().substr(0, 3)+"%");
			$('#killed_ratio').html("R/Tues: "+si.killedRatio.toString().substr(0, 3)+"%");
	...
	gameComplete: function(){
		...
		if(si.shotRatio < si.shotRatioNeeded){
			...
			$('#game_object').append("<div id='info'><p>Votre ratio de monstres tues en tout est trop bas!: "+si.killedRatio+
			...
			$('#score').html("Score: "+si.score);
			$('#game_object').append("<div id='info'><p>Vous winnez finger in ze noze! <br />Ratio tues/balles: "+si.shotRatio+" <br />Ratio tues/vivants: "+si.killedRatio+"<br />Prêt pour le niveau suivant? <br /><a id='restart' href='#'>Allon-y!</a></p></div>");
		}
		$('#restart').click(function(){ si.gameRestart(); });
	},
	...
	gameOver: function(){
		...
		clearInterval(si.myInterval);
		clearInterval(si.starInterval);
		$('#game_object').append("<div id='info'><p>GAME OVER<br /><br /><div class='anim'><a id='restart' href='#'>Recommencer</a></div></p><br /><br />Changer le niveau ? ==>></div>");
		$("input[type=radio]").removeAttr('disabled');
		$("input[type=button]").attr('disabled','disabled');
		si.score = 0;
		si.over = true;
